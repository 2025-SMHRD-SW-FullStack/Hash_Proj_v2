...
const StarRating = ({ score }) => (
  <div className="flex">
    {[1, 2, 3, 4, 5].map((star) => (
      <svg key={star} className={`w-6 h-6 ${star <= score ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-4">
              {imageUrls.map((url, index) => (
                <img key={index} src={url} alt={`피드백 이미지 ${index + 1}`} className="w-40 h-40 rounded-lg object-cover" />
              ))}
            </div>
          </div>
        )}
