    ...
    <header className="sticky top-0 z-50 bg-white shadow-md">
      <div className="flex h-16 sm:h-20 items-center justify-between px-3 sm:px-8">
        ...
        <div className="flex flex-1 items-center space-x-2 sm:space-x-3">
          {isLoggedIn && isAdmin && (
            <Button variant="admin" size="sm" className='sm:size-md' onClick={handleAdminToggle}>
                    {mode === 'user' ? '관리자 페이지' : '유저 페이지'}
                  </Button>
          )}
          {isLoggedIn && isSeller && (
            <Button variant="admin" size="sm" className='sm:size-md' onClick={toggleMode}>
          ...
          {isLoggedIn ? (
            <>
              ...
              <div className="hidden sm:flex items-center space-x-4">
                ...
                <Icon src={User} alt="마이 페이지" className='!h-6 !w-6' onClick={() => navigate('/user/mypage')} />
                <Icon src={Notification} alt="알림" className='!h-6 !w-6' />
                <Icon src={Message} alt="채팅" className='!h-6 !w-6' onClick={() => navigate('/user/chat')} />
                <Icon src={BasketIcon} alt="장바구니" className='!h-6 !w-6' onClick={() => navigate('/user/mypage/cart')} />
